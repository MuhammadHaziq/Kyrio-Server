...
export const checkModules = (req, res, next) => {
  ...
  Users.find({ email: email })
      .then(async (result) => {
        if (result.length > 0) {
          res.status(422).send({ type: "email", message: "An account with this email address already exists" });
        } else {
          if (validator.validate(email)) {
  Modules.findOne()
    .then((result) => {
      if (result != null) {
        Role.findOne({ roleName: "Owner" }).then(roleData => {
          if (roleData != null) {
          ...
          res.status(403).send({ type: "server", message: `Unable to find Role ${err.message}` });
        });
      } else {
        ...
        modules
          ...
          .catch((err) => {
            res
              .status(403)
              .send({ message: `Unable to Save Module ${err.message}` });
          });
      }
    })
    .catch((err) => {
      res
        .status(403)
        .send({ message: `Unable to Find Module ${err.message}` });
    });
    } else {
      res
        .status(422)
        .send({ type: "email", message: "Invalid Email Address" });
    }
  }});
};
